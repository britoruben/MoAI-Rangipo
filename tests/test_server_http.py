...
class _Server:
    ...
    def request(self, method, path, body=None, raw_body=None, headers=None):
        """Returns (status, parsed_json_or_text). A JSON `body` gets a real
        `Content-Type: application/json` header by default, same as every
        fetch() call the actual viewer makes — tests exercising a *wrong*
        or missing Content-Type should use `raw_body` + explicit `headers`
        instead of `body`."""
        headers = dict(headers or {})
        if (body is not None or raw_body is not None) and not any(
...
class TestRuntimeAndPreferencesPost(unittest.TestCase):
    ...
    def test_switching_to_dev_and_back(self):
        with _Server() as s, unittest.mock.patch.object(
            server, "dev_mode_allowed", return_value=True,
        ):
    ...
    def test_malformed_json_returns_400(self):
        # a malformed body is the client's fault -> 400, via _RequestError
        # in _read_body, not an opaque 500
        with _Server() as s:
...
class TestDevExecuteEndpoint(unittest.TestCase):
    ...
    def test_dev_operation_rejected_in_production(self):
        # dev_mode_allowed() true (server started with --dev/MOAI_DEV) but
        # RUNTIME_MODE is still "production": a distinct 409, not the 403
        # dev_mode_allowed() itself would raise if the server weren't even
        # started for Dev mode.
        with _Server() as s, unittest.mock.patch.object(
            server, "dev_mode_allowed", return_value=True,
        ):
    ...
    def test_dev_operation_rejected_when_server_not_started_for_dev(self):
        with _Server() as s:
            status, body = s.request("POST", "/dev/execute",
                                     {"operation": "notes.list", "payload": {}})
        self.assertEqual(status, 403)
...
class TestTtsEndpoint(unittest.TestCase):
    ...
    def test_provider_failure_returns_502(self):
        # real ElevenLabs failures raise _UpstreamError (see text_to_speech),
        # which _guard maps to 502 — not a bare RuntimeError
        with _Server() as s:
            s.galaxy.write_json(s.galaxy.config_el_file, {"api_key": "k", "voice_id": "v"})
            with unittest.mock.patch.object(
                server, "text_to_speech",
                side_effect=server._UpstreamError("quota exceeded"),
            ):
                status, body = s.request("POST", "/tts", {"text": "hola"})
...
class TestChatEndpoint(unittest.TestCase):
    ...
    def test_marker_only_answer_returns_an_error(self):
        # an answer that's only the marker raises _UpstreamError -> 502
        with _Server() as s:
            self._configured(s)
            with unittest.mock.patch.object(
    ...
    def test_placeholder_api_key_asks_for_a_real_one(self):
        # a misconfigured local install is a 500, not a silent 200 (see
        # _handle_chat: answering 200 here made the failure look like a
        # successful chat turn). Covers both the Spanish placeholder that
        # used to be checked AND config.example.json's actual English one —
        # the check used to miss the real template text entirely.
        for placeholder in ("PON-TU-KEY-AQUI", "your-anthropic-key-here"):
            with self.subTest(placeholder=placeholder), _Server() as s:
                s.galaxy.write_json(s.galaxy.config_file, {"api_key": placeholder})
    ...
    def test_api_failure_is_reported_as_an_error_payload(self):
        # real Anthropic failures raise _UpstreamError (see call_claude /
        # _api_request), which _guard maps to 502 — not a bare RuntimeError
        with _Server() as s:
            self._configured(s)
            with unittest.mock.patch.object(
                server, "call_claude",
                side_effect=server._UpstreamError("Anthropic API error: nope"),
            ):
                status, body = s.request("POST", "/chat",
...
class TestUnexpectedFailures(unittest.TestCase):
    ...
    def test_listing_endpoints_report_read_failures(self):
        # a bare RuntimeError is a domain-level failure under _guard, not an
        # unexpected one -> 409, not 500 (see TestGuard in test_server.py)
    ...
    def test_edit_reports_a_failed_rebuild(self):
        # a failed rebuild raises _BuildError under _guard -> 500, not 200
        with _Server() as s:
