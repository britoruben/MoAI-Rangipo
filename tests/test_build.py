...
class TestCollectMdNodes(unittest.TestCase):
    ...
    def test_unreadable_file_is_skipped_not_fatal(self):
        # a single bad-encoding note must not take the whole build down —
        # every other note still builds, with a log_warning instead of a
        # raised exception (same contract as collect_json_nodes)
        with _TempDocs() as td:
            td.write("good.md", "# Good Note\nFine.")
            broken_path = os.path.join(td.root, "broken.md")
            with open(broken_path, "wb") as f:
                f.write(b"\xff\xfe# invalid utf-8 start bytes")
            with td.patch():
                with unittest.mock.patch.object(build, "log_warning") as warn:
                    entries = build.collect_md_nodes()
        self.assertEqual([e["stem"] for e in entries], ["good"])
        self.assertTrue(any("broken.md" in str(c) for c in warn.call_args_list))
