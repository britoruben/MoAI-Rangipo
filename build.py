...
def log_warning(message):
    """Something was skipped but the build went on. stderr, never stdout:
    server.py runs build() in-process and the operator needs to see it."""
    print("MoAI build warning: %s" % message, file=sys.stderr)
...
def read_md(path):
    """Raises (ValueError for bad encoding, OSError for permissions/races)
    rather than returning a sentinel — collect_md_nodes decides how to
    degrade, same split of responsibility as collect_json_nodes."""
    ...
    for dirpath, dirnames, filenames in os.walk(DOCS_DIR):
        ...
        for fn in sorted(filenames):
            ...
            try:
                ...
                # one bad-encoding or permissions-denied note must not take
                # the whole galaxy down — every other note still builds
                log_warning("skipping unreadable %s: %s" % (rel, e))
                continue
            if is_idea:
                node_type = "idea"
            elif is_project:
...
def collect_json_nodes(path, key, node_type):
    ...
    try:
        ...
        log_warning("skipping unreadable %s: %s" % (path, e))
        return []
    if not isinstance(data, dict):
        log_warning("skipping %s: root is not a JSON object" % path)
        return []
    items = data.get(key, [])
    if not isinstance(items, list):
        log_warning("skipping %s: '%s' is not a list" % (path, key))
        return []
...
def build():
    ...
    # atomic write: nobody ever reads a half-written graph-data.js, and a
    # failure halfway through propagates without leaving the .tmp behind
